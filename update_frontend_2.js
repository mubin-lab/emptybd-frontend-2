...
if (content.match(addMemberFuncRegex)) {
  ...
  // Manual replacement if regex fails
...
// Remove the email display (using regex to make sure we match it exactly)
...
const oldAddSectionStart = '{activeConversation.participants.length < 10 && (';
const oldAddSectionEnd = ')}';
const fullAddSectionRegex = /\{activeConversation\.participants\.length < 10 && \(\s*<div className="bg-gray-900\/50 p-4 rounded-xl border border-gray-800">[\s\S]*?<\/div>\s*\)\}/;
