...
export default function CreateProductPage() {
  ...
  // Update owner when user data loads
  useEffect(() => {
    if (user) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (selectedVideo && !formData.video_url) {
      try {
        ...
        console.error("Video upload failed", err);
        toast.error("Video upload failed! Product will be created without video.");
      }
    }
    ...
    if (user) {
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_NODE_API_URL}/e-commerce-product`,
          {
            method: "POST",
            headers: {
      ...
      >
        ...
        <div className="pt-3 md:pt-4">
          <Button
            ...
            type="submit"
            className="w-full bg-black text-white py-2.5 md:py-3 rounded-lg font-medium text-sm md:text-base"
          >
            {loading ? <SpinnerCustom /> : "Create Product"}
          </Button>
        </div>
      </form>
    </div>
